...
export const RodoConsentModal: React.FC<RodoConsentModalProps> = ({ certificateCode, onClose }) => {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg bg-zinc-950 border-2 border-cyan-500/30 rounded-3xl p-8 space-y-6 shadow-2xl">
        <button
        ...
        <p className="text-sm text-zinc-400 leading-relaxed">
          ...
          kursu pojawiły się w publicznej, przeszukiwalnej bazie absolwentów HRL Nova Campus? To
          całkowicie dobrowolne — możesz zmienić zdanie w każdej chwili.
        </p>

        <label
          id="rodo-consent-checkbox-label"
          className="flex items-start gap-3 p-4 bg-zinc-900/60 border border-zinc-800 rounded-2xl cursor-pointer hover:border-cyan-500/40 transition-colors"
        >
          <input
          ...
          <span className="text-xs text-zinc-300 leading-relaxed">
            ...
            publicznej bazie absolwentów HRL Nova Campus.
          </span>
        </label>
        ...
        <button
          ...
          disabled={submitting}
          className="w-full px-5 py-3 bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 disabled:opacity-50 text-white rounded-xl text-xs font-mono font-bold uppercase tracking-wider transition-all cursor-pointer"
        >
